...
class MotionTrackingSampler(PointTrackingSampler):
    ...
    # def __getitem__(self, index):
    #     while True:  # 增加一个循环，用于在get_frames失败时继续尝试获取下一个索引的数据
    #         anno_id = self.get_anno_index(index)
    #         candidate_id = self.get_candidate_index(index)
    #         try:
    #             for i in range(0, self.dataset.get_num_tracklets()):
    ...
    #                     prev_frame_id = max(this_frame_id - 1, 0)
    #                     frame_ids = (0, prev_frame_id, this_frame_id)
    #             print(f"seq_id: {tracklet_id}, frame_ids: {frame_ids}")  # 打印 seq_id 和 frame_ids 的值
    #             first_frame, prev_frame, this_frame = self.dataset.get_frames(tracklet_id, frame_ids=frame_ids)
    #             data = {
    #                 "first_frame": first_frame,
    #                 "prev_frame": prev_frame,
    ...
    #                                     search_transform=self.transform)
    #         except AssertionError:
    #             index = (index + 1) % len(self)  # 如果get_frames失败，就尝试获取下一个索引的数据
    def __getitem__(self, index):
        while True:  # 添加一个循环，用于在get_frames返回空值时跳过该数据???while true?
            anno_id = self.get_anno_index(index)
            candidate_id = self.get_candidate_index(index)
            try:
                for i in range(0, self.dataset.get_num_tracklets()):
                ...
                # 检查get_frames的返回值是否为空
                if self.dataset.get_frames(tracklet_id, frame_ids=frame_ids) is None:
                    print("is none")
                    break
                    raise ValueError("get_frames returned None")
                ...
                return self.processing(data, self.config,
                                    template_transform=self.transform,
                                    search_transform=self.transform)
            except (AssertionError, ValueError):  # 捕获AssertionError和ValueError异常
                print("next")
                # index = (index + 1) % len(self)  # 如果get_frames失败或返回空值，就尝试获取下一个索引的数据
                return self[torch.randint(0, len(self), size=(1,)).item()]
